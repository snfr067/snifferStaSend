...
static void console();
void sendSnifferPkt(void *message);
void setThread();
...
void app_main()
{
    ...
    int err = pthread_attr_init(&t->attr);
    ...
    //pthread_create(&snif_pth_pkt, NULL, sendSnifferPkt, NULL);
    //  
    wifi_init_sniffer();
...
void wifi_sniffer_packet_handler(void* buff, wifi_promiscuous_pkt_type_t type)
{
    ...
    if((send_pkt.timestamp - fstPktTime) < (120*1000000))
    {
        ...
        /*printf("%02X:%02X:%02X:%02X:%02X:%02X, ", hdr->addr1[0], hdr->addr1[1], hdr->addr1[2], hdr->addr1[3], hdr->addr1[4], hdr->addr1[5]);
...
{
    //init_socket_client();
    //printf("TEST THREAD %s\n", (char *)message);
    while(1)
    {
        if(!isSend)
        {
            int boolPhone = (memcmp(scanMac_phone, send_pkt.addr2, sizeof(mac_des)) == 0);
            /*if(boolPhone)
            {
